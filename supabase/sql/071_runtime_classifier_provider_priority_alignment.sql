...
-- "기타 아우터" is deliberately ignored by Swift so the product name can
-- provide a more specific jacket/windbreaker type.
update fitmatch_taxonomy.runtime_classification_rules
set active=false
where rule_set_code='app-hardcoded-parity-2026-08-06-v1'
  and stage='detail' and required_category_code='outerwear' and priority=480;

-- Swift uses sourceText, not the product name, for cropped/7부/9부 pants.
update fitmatch_taxonomy.runtime_classification_rules
set input_scope='source_path'
where rule_set_code='app-hardcoded-parity-2026-08-06-v1'
  and stage='detail' and required_category_code='bottoms'
  and priority in (220,230,240);

update fitmatch_taxonomy.runtime_classification_rules
set include_terms=array['숏','short']
where rule_set_code='app-hardcoded-parity-2026-08-06-v1'
  and stage='detail' and required_category_code='leggings' and priority=602;
...
declare
 ...
 v_deepest text:=lower(trim(regexp_replace(coalesce(p_source_category_path,''),'^.*>','')));
...
begin
 select lower(part) into v_specific
 from unnest(regexp_split_to_array(coalesce(p_source_category_path,''),E'\\s*>\\s*')) with ordinality as d(part,ord)
 where lower(part) ~ '(스커트|skirt|레깅스|leggings|원피스|dress|여성 속옷 하의|팬티|panty|홈웨어|라운지|homewear|loungewear)'
   and lower(trim(part)) not in ('원피스/스커트','원피스 & 스커트','속옷/홈웨어','속옷 & 홈웨어')
 order by ord desc limit 1;
 if v_specific is null then
  v_specific:=case when lower(coalesce(p_source_category_path,'')) ~ '(원피스/스커트|원피스 & 스커트|속옷/홈웨어|속옷 & 홈웨어)'
   then '' else lower(coalesce(p_source_category_path,'')) end;
 end if;
 select lower(part) into v_deepest
 from unnest(regexp_split_to_array(coalesce(p_source_category_path,''),E'\\s*>\\s*')) with ordinality as d(part,ord)
 where lower(part) ~ '(패딩|베스트|조끼|가디건|카디건|cardigan|블레이저|blazer|블루종|blouson|플리스|fleece|아노락|anorak|바람막이|윈드브레이커|windbreaker|무스탕|mouton|트렌치|trench|파카|parka|코트|coat|점퍼|후드 집업|후드집업|후디|jumper|재킷|자켓|jacket)'
 order by ord desc limit 1;
 v_deepest:=coalesce(v_deepest,'');
 ...
 loop
  ...
  v_candidate:=case when v_rule.stage='detail' and v_category='outerwear' then v_deepest
   else case v_rule.input_scope when 'specific_source' then v_specific when 'source_path' then v_path
   when 'product_name' then v_name when 'category_code' then coalesce(v_category,'')
   when 'detail_code' then coalesce(v_detail,'') else concat_ws(' ',v_path,v_name) end end;
