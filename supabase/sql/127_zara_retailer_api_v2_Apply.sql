-- DEVELOPMENT candidate; MANUAL apply by the user. Remote writes by auditor: 0.
...
DECLARE
 ...
 version_value text := 'fitmatch-vnext-retailer-api-20260909-r1';
 status_value text; reason_value text; candidate_values jsonb := '[]'::jsonb;
 tuple_value jsonb; fixed_value jsonb; unknown_value jsonb;
 conflict boolean := false; product_specific boolean := false; invalid_evidence boolean := false;
BEGIN
...
select case
 ...
 IF product_row.source_code='zara'
    AND envelope->>'contract_version'='fitmatch-retailer-api-v2'
 THEN version_value:='fitmatch-vnext-retailer-api-20260909-r2'; END IF;
    ...
    SELECT api_facts||coalesce(jsonb_agg(f),'[]'::jsonb) INTO api_facts FROM (
     ...
     FROM (VALUES (api_product->>'description','$.product.description'),
       ...
       (coalesce(api_selected->>'rawDescription',api_selected->>'description'),
          '$.product.detail.colors[productId='||api_selected_key||'].description')) d(v,p)
...
begin
    ...
    if product_row.resolver_version in (
      'fitmatch-vnext-retailer-api-20260909-r1',
      'fitmatch-vnext-retailer-api-20260909-r2') then
      declare
        ...
        contract_value text := case product_row.resolver_version
          when 'fitmatch-vnext-retailer-api-20260909-r2'
            then 'fitmatch-vnext-recovery-retailer-api-20260909-r2'
          else 'fitmatch-vnext-recovery-retailer-api-20260909-r1' end;
      begin
...
DO $post$ BEGIN
 IF NOT coalesce((EXISTS(SELECT 1 FROM pg_proc WHERE oid=to_regprocedure('fitmatch_vnext.classification_decision(text,text)') AND md5(replace(prosrc,chr(13),''))='5a81fa24b06a3aa43f7a9ffab553788a')),false) THEN RAISE EXCEPTION 'FM_PROVIDER_POSTCHECK: procedure_1_installed'; END IF;
 ...
 IF NOT coalesce((EXISTS(SELECT 1 FROM pg_proc WHERE oid=to_regprocedure('fitmatch_vnext.classification_recovery_options(uuid)') AND md5(replace(prosrc,chr(13),''))='dca1dda221090e661628ffb5811e13ef')),false) THEN RAISE EXCEPTION 'FM_PROVIDER_POSTCHECK: procedure_2_installed'; END IF;
...
SELECT 'procedure_1_installed' check_name, coalesce((EXISTS(SELECT 1 FROM pg_proc WHERE oid=to_regprocedure('fitmatch_vnext.classification_decision(text,text)') AND md5(replace(prosrc,chr(13),''))='5a81fa24b06a3aa43f7a9ffab553788a')),false) passed
UNION ALL
SELECT 'procedure_1_privileges' check_name, coalesce((EXISTS(SELECT 1 FROM pg_proc WHERE oid=to_regprocedure('fitmatch_vnext.classification_decision(text,text)') AND proacl::text='{postgres=X/postgres,service_role=X/postgres,authenticated=X/postgres,anon=X/postgres}' AND prosecdef=false AND proconfig=ARRAY['search_path=""']::text[])),false) passed
UNION ALL
SELECT 'procedure_2_installed' check_name, coalesce((EXISTS(SELECT 1 FROM pg_proc WHERE oid=to_regprocedure('fitmatch_vnext.classification_recovery_options(uuid)') AND md5(replace(prosrc,chr(13),''))='dca1dda221090e661628ffb5811e13ef')),false) passed
UNION ALL
