...
begin
  ...
  if grouped is null
     or nullif(btrim(grouped->>'group_code'),'') is null then
    return detail;
  end if;
  ...
  if nullif(btrim(group_policy),'') is null then
    -- Missing group policy must not prevent explicit classification input.
    -- Preserve validated personal/global authority or the legacy review state.
    return detail;
  end if;
