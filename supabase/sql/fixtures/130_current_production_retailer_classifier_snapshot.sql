-- READ-ONLY snapshot of the currently deployed Production functions.
-- Captured 2026-09-10 for disposable/local before-after classification testing.
-- Do not apply this file to Production. Contains function definitions only; no rows.
...
BEGIN
 ...
 WITH rules AS (
   ...
   CROSS JOIN LATERAL (
       SELECT value_code FROM axes
       WHERE axis_code='sleeve_length' AND g.uses_sleeve_length
         AND EXISTS(SELECT 1 FROM hits WHERE dimension='sleeve_length_code')
       UNION ALL SELECT NULL
       WHERE NOT g.uses_sleeve_length
          OR NOT EXISTS(SELECT 1 FROM hits WHERE dimension='sleeve_length_code')) s
   CROSS JOIN LATERAL (
       SELECT value_code FROM axes
       WHERE axis_code='lower_length' AND g.uses_lower_length
         AND EXISTS(SELECT 1 FROM hits WHERE dimension='lower_length_code')
       UNION ALL SELECT NULL
       WHERE NOT g.uses_lower_length
          OR NOT EXISTS(SELECT 1 FROM hits WHERE dimension='lower_length_code')) l
   CROSS JOIN LATERAL (
       SELECT value_code FROM axes
       WHERE axis_code='body_length' AND g.uses_body_length
         AND EXISTS(SELECT 1 FROM hits WHERE dimension='body_length_code')
       UNION ALL SELECT NULL
       WHERE NOT g.uses_body_length
          OR NOT EXISTS(SELECT 1 FROM hits WHERE dimension='body_length_code')) b
 ...
 SELECT jsonb_build_object(
   ...
   'resolver_version','fitmatch-provider-api-20260909-r3','policy_version',policy_value->>'version',
...
begin
    ...
    if product_row.resolver_version in ('fitmatch-vnext-retailer-api-20260909-r1',
                                      'fitmatch-vnext-retailer-api-20260909-r2',
                                      'fitmatch-vnext-retailer-api-20260909-r3') then
      declare
        ...
        contract_value constant text := 'fitmatch-vnext-recovery-retailer-api-20260909-r3';
      begin
