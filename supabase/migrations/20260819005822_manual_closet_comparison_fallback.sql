...
begin
  ...
  if not found or v_target.classification_status <> 'confirmed' then
    return jsonb_build_object(
      'state','target_classification_required','automatic_count',0,'manual_count',0,
      'structural_count',0,'candidates','[]'::jsonb,
...
begin
  ...
  if p_client_history_id is null then raise exception using errcode='22023',message='client_history_id_required'; end if;
  ...
  if not found then raise exception using errcode='P0002',message='reference_item_not_found'; end if;
  ...
  if not found or v_target.classification_status<>'confirmed' then
    v_compatibility:=jsonb_build_object(
      'allowed',false,'level','incompatible','reason','target_classification_not_confirmed',
      'excluded_measurements','[]'::jsonb
    );
  elsif v_reference.family_code is null then
    v_compatibility:=jsonb_build_object(
      'allowed',false,'level','incompatible','reason','reference_classification_not_confirmed',
      'excluded_measurements','[]'::jsonb
    );
  else
    v_compatibility:=fitmatch_catalog.runtime_evaluate_comparison_profiles_v3(
      ...
      v_target.category_code,v_product.audience,v_target.comparison_family_code,
      v_target.detail_code,v_target.length_code,v_target.body_length_code,p_allow_extended
    );
    if coalesce((v_compatibility->>'allowed')::boolean,false)
       and fitmatch_catalog.runtime_closet_measurement_overlap(
         p_reference_item_id,p_target_product_id,v_compatibility->'excluded_measurements'
  ...
  insert into public.comparison_runs(
    ...
    comparison_level,block_reason,comparison_policy_version,input_snapshot,completed_at
  ) values (
    v_user_id,p_client_history_id,p_reference_item_id,p_target_product_id,v_status,
    v_compatibility->>'level',v_compatibility->>'reason','db-comparison-2026-08-19-v3',
    jsonb_build_object(
      'compatibility',v_compatibility,'client_history_id',p_client_history_id,
...
grant execute on function public.fitmatch_find_reference_candidates(uuid) to authenticated,service_role;
revoke all on function public.fitmatch_begin_comparison(uuid,uuid,boolean,uuid) from public,anon;
grant execute on function public.fitmatch_begin_comparison(uuid,uuid,boolean,uuid) to authenticated,service_role;
