...
create or replace function fitmatch_catalog.runtime_ingest_product_payload(p_payload jsonb)
returns jsonb language plpgsql security invoker
set search_path=pg_catalog,fitmatch_catalog as $$
declare
 v_product_id uuid; v_variant jsonb; v_size jsonb; v_measurement jsonb;
 v_variant_id uuid; v_size_id uuid; v_category_scope text;
 v_variants integer:=0; v_sizes integer:=0; v_measurements integer:=0;
begin
 ...
 for v_variant in select value from jsonb_array_elements(
   case when jsonb_typeof(p_payload->'variants')='array' then p_payload->'variants'
   else jsonb_build_array(jsonb_build_object('external_variant_id','__default__','sizes',coalesce(p_payload->'sizes','[]'::jsonb))) end)
 loop
  v_variant_id:=fitmatch_catalog.runtime_upsert_variant(v_product_id,v_variant);
  v_variants:=v_variants+1;
  for v_size in select value from jsonb_array_elements(
    case when jsonb_typeof(v_variant->'sizes')='array' then v_variant->'sizes' else '[]'::jsonb end)
  loop
   ...
   for v_measurement in select value from jsonb_array_elements(
     case when jsonb_typeof(v_size->'measurements')='array' then v_size->'measurements' else '[]'::jsonb end)
   loop
    perform fitmatch_catalog.runtime_upsert_measurement(v_size_id,
      case when nullif(v_measurement->>'category_scope','') is not null or v_category_scope is null
      then v_measurement else v_measurement||jsonb_build_object('category_scope',v_category_scope) end);
    v_measurements:=v_measurements+1;
   end loop;
  end loop;
 end loop;
 return jsonb_build_object('product_id',v_product_id,'variants_processed',v_variants,'sizes_processed',v_sizes,'measurements_processed',v_measurements);
end $$;

create or replace function public.fitmatch_batch_ingest_product(p_payload jsonb)
returns jsonb language plpgsql security invoker set search_path=pg_catalog as $$
declare v_ingest jsonb; v_promotion jsonb; v_comparison_ready boolean;
begin
 if jsonb_typeof(p_payload)<>'object' then raise exception using errcode='22023',message='payload_must_be_object'; end if;
 if coalesce(p_payload->'raw_payload'->>'batch_ingest_version','')='' then raise exception using errcode='22023',message='batch_ingest_version_required'; end if;
 ...
 select (v_promotion->'classification'->>'classification_status'='confirmed') and exists(
  ...
  join fitmatch_catalog.product_measurements m on m.product_size_id=s.id and m.is_comparable
...
insert into fitmatch_taxonomy.source_measurement_aliases(
 source_code,parser_code,raw_code,raw_label,normalized_raw_label,measurement_code,
 raw_representation,comparison_basis,conversion_multiplier,category_scopes,is_comparable,evidence,policy_version)
values
...
on conflict(source_code,raw_label,measurement_code,policy_version) do update set
 parser_code=excluded.parser_code,raw_code=excluded.raw_code,normalized_raw_label=excluded.normalized_raw_label,
 raw_representation=excluded.raw_representation,comparison_basis=excluded.comparison_basis,
 conversion_multiplier=excluded.conversion_multiplier,category_scopes=excluded.category_scopes,
 is_comparable=excluded.is_comparable,evidence=excluded.evidence;

revoke all on function fitmatch_catalog.runtime_ingest_product_payload(jsonb) from public,anon,authenticated;
grant execute on function fitmatch_catalog.runtime_ingest_product_payload(jsonb) to service_role;
revoke all on function public.fitmatch_batch_ingest_product(jsonb) from public,anon,authenticated;
grant execute on function public.fitmatch_batch_ingest_product(jsonb) to service_role;
commit;;
