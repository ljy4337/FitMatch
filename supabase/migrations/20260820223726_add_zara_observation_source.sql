...
select pg_advisory_xact_lock(hashtext('fitmatch:add-zara-observation-source-v1'));

-- ZARA is accepted only at the existing observation boundaries. This does not
-- expose a new public table or relax RLS; users still require authentication.
...
alter table fitmatch_catalog.product_observations
  ...
  check (source in ('uniqlo', 'musinsa', 'cos', 'zara'));
...
begin
  ...
  if v_source not in ('uniqlo', 'musinsa', 'cos', 'zara') then
