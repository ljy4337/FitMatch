alter table fitmatch_vnext.manual_cross_comparison_rules
add column if not exists require_same_sleeve boolean not null default true;

update fitmatch_vnext.manual_cross_comparison_rules
set require_same_sleeve = true,
    updated_at = now()
where (policy_code_a, policy_code_b) in (
...
begin
    ...
    if not structural_ok and p_manual_explicit then
        ...
        manual_cross_allowed := coalesce(manual_cross_allowed, false)
            and not (coalesce(cross_requires_same_sleeve, false) and sleeve_mismatch);
