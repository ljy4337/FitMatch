...
-- PostgREST exposes public, not fitmatch_catalog.  This service-role-only RPC
-- keeps the existing private ingest and classification functions atomic.
...
-- The batch asks in bounded chunks which observed products have not yet been
-- ingested by its current payload contract.  Existing products are not
-- downloaded again merely because they already exist in the local ledger.
