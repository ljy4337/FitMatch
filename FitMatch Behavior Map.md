...
Payload owners: `closetCreationPayload` and linked `closetUpdatePayload` both opt into the deployed `apply_linked_closet_snapshot_for_swift` contract (`use_server_measurements=true`, empty client canonical array). Manual create/update retains the explicit measurement contract. Server still checks auth, exact hierarchy, group, semantic conflicts and canonical completeness. Preserve request fingerprint/idempotency. Review/apply/postflight: `supabase/sql/core_registration_candidate_contract_Apply.sql` / `_Verify.sql`.
...
public.fitmatch_vnext_comparison_history → local projection → sort/filter/detail → recompare or result Closet registration. `ContentFilterBar` uses fixed-width, left-truncated controls and keeps the layout toggle outside the filter scroller, so selection labels cannot move it. History-to-Closet preparation uses one in-flight request token for the single registration sheet. Different-history taps cannot start overlapping preparations; disappear/account changes cancel and invalidate pending work, and stale completions cannot present or release a newer request. Owner: FitMatchResultClosetRegistrationPreparationAction.swift / RecommendationHistoryView.swift. Visibility action uses public.fitmatch_vnext_hide_comparison_history. Transport: FitMatchSupabaseProductResolver. Local removal alone is not server deletion proof.
