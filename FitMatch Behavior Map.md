...
### DEBUG server latency diagnostics

`FitMatchSupabaseDomainClient.timedDatabaseCall` records only a fixed Edge/RPC operation name, completion state, request origin/short trace ID, and elapsed milliseconds. `FitMatchRequestTrace` connects the foreground product-load, comparison, Closet-registration, and background-sync operations; payloads, product data, user IDs, and measurements are excluded. Segment timings include share-link entry→candidate display, candidate preparation, selected Closet item→begin, result save, linked registration, and background sync. Use these logs to identify a slow server boundary before changing UI or database behavior.
...
Provider resolver → details/measurements → ParsedProductInfo → fitMatchProductObservationRequest() → resolveFreshRetailerProductAuthority() → product-observation Edge → public.fitmatch_vnext_ingest_product_observation → internal ingestion/v2 → public.fitmatch_vnext_get_product_runtime → get_product_runtime_for_swift → DTO/contract validation → validated UI state. After exact identity resolution, UNIQLO starts distinct preferred/generic charts and product/stock reads together (identical chart URLs are requested once); its established chart-selection and UNKNOWN-stock rules apply only after receipts arrive. MUSINSA starts metadata and raw actual-size receipts together after the exact product ID resolves, then parses actual size only with the existing metadata category context; a transport cancellation is not HTML recovery. ZARA starts details as soon as the product page verifies the exact catalog entry; the concurrent selected `v1` guide is consumed only when that identity matches. Parent cancellation while the page is pending immediately cancels that speculative guide, so neither details nor an exact-guide recovery begins. When a verified redirect selects a different catalog entry, cancel/discard the obsolete guide and start the exact guide without waiting for it. DEBUG spans retain the request trace/origin across provider HTTP, parser, observation payload, Edge/RPC boundaries without recording raw URL, product, user, or measurement payloads; use wall-clock spans, not summed parallel request durations.
...
FLOW-PRODUCT-LOAD → exact selected product/variant/size → linked creation payload (`use_server_measurements=true`, `measurements=[]`) → public.fitmatch_vnext_upsert_closet_item → server canonical snapshot → authoritative success/read-back → cache sync. LinkClosetRegistrationView keeps the retailer card hidden until this final state; loading never enables `다음`. During loading, its load button observes the existing ProductAnalysisPhase through FitMatchLinkClosetRegistrationAction: retailer product/size collection shows `쇼핑몰 정보 불러오는 중`, authority preparation shows `상품 정보 확인 중`. URL editing is disabled until loading ends. CompareFlowSheet uses the same phase copy; no estimated duration is displayed. `MeasurementResolver.sourceDisplayRows` owns registration display of every exact retailer row; canonical rows must not replace or coalesce raw rows. `20260921110000_closet_raw_measurement_snapshots.sql` prepares immutable linked Closet raw snapshots in `closet_item_source_measurements`; apply state must be verified separately. If the automatic group is unchanged, omit comparison_group_code; if unresolved, require explicit A–G choice. Linked Closet size menus display every received Product size, preserving exact IDs even when registration evidence is missing. AddComparedProductToClosetSheet.selectableSizes no longer filters by isRegisterable; selected-size guidance and save-time eligibility/identity checks remain separate. Inspect ShoppingProductViewModel and SupabaseProductResolver for payload/identity contracts. Storage: closet_items + closet_item_measurements + (prepared) closet_item_source_measurements.

Payload owners: `closetCreationPayload` and linked `closetUpdatePayload` both opt into the deployed `apply_linked_closet_snapshot_for_swift` contract (`use_server_measurements=true`, empty client canonical array). A linked size change additionally carries the exact `source_observation_id`; the server must prove that receipt's product/variant/size before it atomically replaces parent identity, canonical rows, and raw source snapshot rows. A metadata-only linked edit never replaces raw evidence. Read-back verifies the requested receipt and every raw row before UI success. Manual create/update retains the explicit measurement contract. An explicit `closet_detail_code_snapshot` is separate from the coalesced legacy detail code: it round-trips an exact supported user choice, while snapshot-absent rows retain tuple-based legacy recovery. Server still checks auth, exact hierarchy, group, semantic conflicts and canonical completeness. Preserve request fingerprint/idempotency. Local migration candidates: `20260924100000_linked_closet_size_snapshot_updates.sql` and `20260924101000_closet_detail_snapshot_round_trip.sql`; they are not an apply record.

### FLOW-CLOSET-MANUAL — 직접 입력 → 내 옷장 추가

READ FIRST: `FitMatch/Views/AddClosetItemView.swift`, `FitMatch/ViewModels/AddClosetItemViewModel.swift`, `FitMatch/Services/FitMatchClosetSyncCoordinator.swift` (`registerManualServerFirst`), `FitMatch/Services/FitMatchClosetFormAction.swift` (editing).

New manual registration from Closet/Compare/Onboarding: validated detached draft → existing manual payload → public.fitmatch_vnext_upsert_closet_item → ready list receipt with exact identities → authoritative local projection → onSaved/dismiss. No local-only success. Ambiguous failures retain the same draft/request for read-back retry; definite rejection leaves input editable. Background sync and registration share the coordinator mutation lock. Numeric input uses the same whitespace normalization for validation and stored values. The new-manual UI presents gender plus the seven linked-registration group labels; it hides source, brand, product-name, and detail inputs while retaining the compatible internal taxonomy tuple. Legacy synchronous FitMatchClosetManualRegistrationAction is not the new registration persistence authority. Do not fabricate linked retailer identities.
...
public.fitmatch_vnext_list_closet_items → cache projection → exact item edit via fitmatch_vnext_update_closet_item / delete via fitmatch_vnext_delete_closet_item → authoritative success → cache update. `FitMatchClosetSyncCoordinator` shares an in-flight pass for the same user plus semantic local fingerprint, skips linked server-authoritative rows with no client-owned change, and only re-lists after a real delete/mutation. A deterministic linked payload contract rejection preserves local input and pauses repeat uploads until content changes or an explicit retry clears the guard. Linked M→L does not succeed from a latest/label-selected receipt: it requires the exact source observation, performs the parent/canonical/raw swap atomically, and verifies the source snapshot on list read-back. Explicit supported manual detail values are checked against `closet_detail_code_snapshot`; a coalesced server `closet_detail_code` alone is not proof of the user selection. Classification override set/clear transport remains in SupabaseProductResolver; verify actual call sites before treating it as current exposed UX or a removal candidate.
...
URL → FLOW-PRODUCT-LOAD → mapped group or explicit session group → FLOW-CANDIDATES → user picks a same-group Closet item → eligible sizes / authorization / begin → engine → complete → result/history. After target authority and the server Closet/candidate plan are confirmed, show the terminal empty-Closet notice only when both local active rows and the verified server Closet receipt are empty. A missing/unready receipt or a missing local candidate projection uses the existing retry/error path, never a false empty-Closet dismissal. Candidate lookup retains its normal authority checks; no eligible/begin work runs for an empty Closet. A nonempty Closet with no server-approved candidate ends with an explanatory message and one dismiss action; it does not enter registration or other-product comparison. Comparison summary accessibility labels include the owned size to distinguish multiple Closet entries for the same product. Summary cards explicitly label local rankings/sizes as estimates; detail reauthorizes and computes from the server snapshot, so numeric parity is not yet verified. Card navigation hints/chevrons must not deny server-approved candidates solely because local preview evidence is insufficient. Never display a successfully saved result before completion persistence is confirmed.

### FLOW-CANDIDATES — 비교할 내 옷 후보

2026-09-23 authorized development DB deployment: `same_comparison_group_only_preserve_eligibility` applied to hnkplvyegonlhumlejst. Native/session candidate and authorization group gates verified by definition inspection; existing deployed eligible-size validation preserved. Authenticated comparison E2E not run in this deployment.


`resolveWithRuntime` carries the fresh server runtime alongside resolution within one authority call. Reuse is allowed only before any promotion; mutations require a new runtime read. No cross-request cache or candidate/begin validation bypass.
...
referenceSelectionPlan() starts only independent reads (target authority and current Closet receipt) together; after target/session identity is verified it calls public.fitmatch_vnext_find_reference_candidates → fitmatch_vnext.find_reference_candidates → server-filtered list. Swift displays approved candidates; user chooses one. Empty candidates are an explicit state, not permission for local fallback. Eligible sizes, authorization, and begin remain ordered server-authority gates.
...
Mapped 2-argument candidates call `eligible_candidate_sizes` for each exact target variant and return only authorized same-group Closet item IDs; zero eligible sizes exclude that Closet candidate. The requested-group overload retains its session-context validation and accepts only the server-validated session group. Candidate eligibility still requires >=1 common canonical policy metric; begin reauthorization remains mandatory. A↔B domain repair (`unclassified_outerwear` is UPPER_BODY) remains a measurement-domain rule, not permission for a cross-group comparison.

The 2026-09-23 policy supersedes the 2026-09-16 explicit-cross-group allowance. `CompareFlowSheet` has no other-group candidate picker or candidate-empty registration/restart action. `FitMatchServerAuthorityCoordinator` verifies every selectable VNext candidate against the current authoritative Closet receipt group before it can reach authorization. Server follow-up owner: `supabase/migrations/20260923110000_same_comparison_group_only.sql`; verification/rollback: `supabase/sql/20260923_same_comparison_group_only_Verify.sql`, `supabase/sql/20260923_same_comparison_group_only_Rollback.sql`; isolated regression: `supabase/sql/tests/same_comparison_group_only_Regression.sql`.
...
The legacy list RPC returns active rows only; it is not authority to delete an absent local row. Where deployed, public `fitmatch_vnext_comparison_history_sync` returns active rows plus owner-scoped exact tombstones. The sync coordinator persists a tombstone only after local cache save, filters an already-in-flight stale completed row before hydration, and never removes shared Product/Closet rows. Old servers returning only the legacy list remain read-compatible but cannot propagate a hide to another device. `ContentFilterBar` uses fixed-width, left-truncated controls and keeps the layout toggle outside the filter scroller, so selection labels cannot move it. History-to-Closet preparation uses one in-flight request token for the single registration sheet. Different-history taps cannot start overlapping preparations; disappear/account changes cancel and invalidate pending work, and stale completions cannot present or release a newer request. Owner: FitMatchResultClosetRegistrationPreparationAction.swift / RecommendationHistoryView.swift. Visibility action uses public.fitmatch_vnext_hide_comparison_history. Transport: FitMatchSupabaseProductResolver. Local removal alone is not server deletion proof. The additive tombstone migration candidate is `20260924102000_comparison_history_tombstone_sync.sql`; it is not deployed by this map.
...
Closet `mapClosetItem` retains both immutable source records (unknown/non-comparable) and separate `fitmatch_vnext_snapshot` canonical records. `MeasurementResolver.sourceDisplayRows` displays the original records without duplicating canonical projections; canonical-only legacy rows remain visible. Local candidate previews consume verified canonical records, while final comparison still consumes only server begin evidence. An unavailable preview is shown as `선택 후 확인`, never as the server's minimum-measurement rejection.
...
- Once the exact product ID is resolved, start those two official HTTP receipts together. Retain the raw actual-size response until metadata supplies the existing upper/lower semantic context; metadata/actual failure capture and HTML/image recovery remain unchanged, but `CancellationError`/`URLError.cancelled` must terminate rather than start HTML recovery.
...
- For different official chart URLs, start preferred and generic receipts together, and start exact product/stock availability reads alongside them. Request a shared chart URL once. Apply the existing larger-chart/tie selection and stock annotations only after all required receipts settle. Product/stock transport is typed-data-only and must not manufacture an unused generic JSON projection; size-chart/evidence captures retain their immutable projection.
