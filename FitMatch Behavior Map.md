...
public.fitmatch_vnext_comparison_history → local projection → sort/filter/detail → recompare or result Closet registration. `ContentFilterBar` uses fixed-width, left-truncated controls and keeps the layout toggle outside the filter scroller, so selection labels cannot move it. Visibility action uses public.fitmatch_vnext_hide_comparison_history. Transport: FitMatchSupabaseProductResolver. Local removal alone is not server deletion proof.
