...
## 2026-09-08 Simple FitMatch Phase 1 — linked Closet local snapshot / provenance round-trip

- Started on requested `simplefitmatch` after a read-only `git fetch origin simplefitmatch`; local and `origin/simplefitmatch` both resolved to `82b6c8e161f2f5b2062a5513c110aa48d330b3bc`. The three pre-existing untracked owner files (`FitMatchTests/MeasurementResolverDisplayTests.swift` and the two `20260903` Supabase migrations) were preserved. No reset/stash/clean/delete, commit, push, Production DB/RPC/data write, migration application, deployment, or account mutation occurred.
- The link registration surface now requires an explicit active FitMatch category/detail selection, preserves the complete server size list (including no-measurement sizes as non-registerable), selects an exact Product/variant/size identity, and presents a Closet-local editable snapshot seeded from the selected size chart. It reuses the existing taxonomy and measurement definitions; no string/label heuristic or new taxonomy was added.
- The snapshot retains every verified retailer fact as `RETAILER_SNAPSHOT`; editing or adding one definition changes only that row to `USER_MANUAL`. `FitMatchSupabaseDomainClient` sends linked measurements, serializes per-row `value_source` and `source_measurement_code`, and restores those fields through DTO → `UserFit.measurementRecords`. Shared `Product` / `ProductSize` instances are not mutated by the edit path.
- Linked sync keeps exact remote product/variant/size validation, ownership, auth, stale-session, retry, and idempotency checks, but does not re-resolve automatic Product classification when an existing linked Closet item has an explicit personal classification. This prevents an automatic reclassification from overwriting or blocking the chosen Closet tuple.
- `supabase/sql/127_simplefitmatch_linked_closet_round_trip_candidate.sql` and its validation companion are **candidate-only**, not a migration and not applied anywhere. Source audit found the existing `closet_items.measurement_mode` and `closet_item_measurements.value_source` can represent the mixed snapshot, so no schema column is proposed. The candidate preserves exact hierarchy and RLS/ownership checks, validates finite positive cm values and active canonical codes, retains every canonical selected-size code, and returns per-row provenance/raw source identity from list. No configured local/Production server was available (`supabase` CLI absent; only `psql` client installed; local `initdb` cannot start without `postgres`), therefore disposable SQL execution is **NOT RUN** and Production was not used as a substitute.
- Focused current-source XCTest evidence: linked round-trip regression `9 PASS / 0 FAIL`; Closet transport + sync contracts `43 PASS / 0 FAIL`; combined final run `52 PASS / 0 FAIL / 0 SKIP` at `/tmp/FitMatchSimpleFitMatchFinalRegression.xcresult`. These cover untouched imported rows, one-field 55→56 mixed provenance, addition, DTO hydration/re-hydration, personal category authority, sync non-reclassification, exact identity, a later exact-size change carrying the selected retailer snapshot, shared-data isolation, invalid values, and manual regression. `FitMatchSupabaseProductResolverTests` is `44 PASS / 5 FAIL`; the exact same five Result→Closet preferred-size/variant tests reproduce from a clean archive of `82b6c8e`, so they are baseline failures rather than claimed fixed/regressed. Full suite and a real Swift→RPC→DB→list round trip remain unproven until the candidate is exercised against a disposable Supabase database.
- No Comparison Engine score/ranking/reliability, Result/History policy, parser extraction, global automatic-classification assets, or protected TabBar/scroll code was changed.
- Final static checks: `git diff --check` passed; `FitMatch/Components/TabBarScrollVisibilityModifier.swift` has no diff; protected scroll-modifier grep produced no matches (expected exit `1`).

## 2026-09-09 Simple FitMatch Phase 1 — actual disposable PostgreSQL round-trip closure

- Preflight began from a clean `simplefitmatch` worktree after `git fetch origin simplefitmatch`; local `HEAD`, `FETCH_HEAD`, and `origin/simplefitmatch` all resolved to `79020702dd1c1253e4a6012d328aa7b512148706` (`simple v1`). No reset/stash/clean, commit, push, Production DB/RPC/data write, migration apply, deployment, or account mutation occurred.
- Linked Closet re-editing now owns a Closet-local mixed snapshot. A category-only edit starts from the current `UserFit` records (so an earlier USER_MANUAL chest remains intact); a real exact-size change starts from that selected `ProductSize` chart. `FitMatchClosetSyncCoordinator` sends the editor snapshot verbatim instead of reconstructing it from the shared chart. The imported Closet detail sheet now exposes the same category-definition measurement editor as link registration, displays all retail sizes, and disables only sizes with no positive garment measurement.
- Exact semantic handling was tightened without a display-label heuristic: a circumference fact cannot seed a width field. An added direct field keeps its existing method-specific local code and is normalized only by `FitMatchCanonicalMeasurementCode` at transport. The scalar compatibility map now uses that exact canonical map first, preventing a duplicate `waist_width_edge_to_edge`/`waist_width` representation while retaining raw source identity in records. Untouched `55.25` stays `55.25 RETAILER_SNAPSHOT`; it is no longer rounded to `55.2` and falsely converted to USER_MANUAL.
- Transport/hydration preserves per-row `value_source` and `source_measurement_code`; direct values validate only against the exact existing FitMatch definition, not a shared display axis. An unrated local `satisfaction == 0` is omitted and decodes back as `0`, rather than being fabricated as `3`.
- `supabase/sql/127_simplefitmatch_linked_closet_round_trip_candidate.sql` remains **candidate-only, not a migration**. The existing `measurement_mode` and `value_source` shape is sufficient; no schema column is proposed. The candidate now fixes a PL/pgSQL measurement-code shadowing bug, keeps the public list bridge aligned with enriched per-row source identity, validates the mixed snapshot before replacement, and preserves direct-input routing through the existing `upsert_closet_item_for_swift` path. `127_simplefitmatch_linked_closet_round_trip_local_preimage.sql` is a local-fixture-only catalog/check preimage, never a Production artifact.
- A disposable PostgreSQL 17 server was started under `/tmp`, received only the local fixture, the repository's existing `20260829012836_vnext_closet_reference_api.sql` and `20260829110853_vnext_swift_user_contract.sql` migration source, local preimage, and candidate SQL, and was then stopped cleanly. The fixture omits one pre-existing comparison function required only to compile a public wrapper, so the preimage supplies a local-only signature shim that validation never invokes; the untouched Swift-user migration then completed end-to-end. Applying the candidate afterwards plus `127_simplefitmatch_linked_closet_round_trip_local_validation.sql` passed actual public-RPC create → update → list/read checks for untouched retailer rows, 55.25→56 USER_MANUAL chest (including its `source_measurement_code`), retained imported shoulder/back/sleeve values, added user hem (including its separate source identity), category-only change, post-save re-edit, exact identity, cross-user rejection, invalid-unit rejection, Product/ProductSize immutability, and outer transaction rollback. A post-run read check found `0` synthetic linked rows, `0` persisted linked fixture rows, and the original ProductSize measurement remained `50`; `authenticated` retained RPC execute while `anon` did not. No Production connection was available or used.
- Scoped current-source XCTest on iPhone 17 Simulator (`FitMatchClosetTransportContractTests`, `FitMatchClosetSyncCoordinatorTests`, `FitMatchLinkedClosetRoundTripTests`) is **56 PASS / 0 FAIL / 0 SKIP**, final result bundle `/tmp/FitMatchSimpleFitMatchRoundTripFinal2.xcresult`. The first run exposed two real test observations (scalar duplicate projection and local-method-code vs server-canonical assertion); both were corrected and rerun green. The pure taxonomy/measurement lookups used by the nonisolated payload builder are explicitly nonisolated now; no measurement definition or mapping changed. The existing broad `123_vnext_swift_user_contract_tests.sql` could not run against this intentionally linked-only fixture because it first requires an unrelated `fitmatch_vnext.get_product_runtime_for_swift(text,text)` function; that is recorded as NOT RUN, not a pass. Direct-input transport and authority regressions are covered by the green scoped XCTest, and the linked candidate delegates non-linked requests unchanged.
- Final gates passed: `git diff --check`, the protected TabBar file diff, and the protected scroll-symbol grep are clean. Production read-only schema/function verification remains unavailable because no Production DSN/credential was configured; do not apply the candidate until an authorized read-only schema/RPC comparison confirms it matches the live contract.

## 2026-09-09 Simple FitMatch Phase 1 — final exact-identity/UI audit follow-up

- The `NOT_COMPARABLE` regression now supplies a real exact runtime Product/variant/size tuple and one canonical measurement. The Simple link preparation permits explicit user-Closet registration in that state, while a transport failure with no exact identity remains blocked. This proves the distinction is classification-only, not a relaxation of hierarchy identity checks.
- Both the initial registration sheet and imported linked-item edit sheet now render the selected Closet-local snapshot as the TextField fallback before `@State` promotion. API/retailer values therefore appear on the first frame; the first edit promotes that same snapshot and does not mutate the shared `ProductSize`.
- Final current-source Simulator execution completed with **57 PASS / 0 FAIL / 0 SKIP**: `FitMatchClosetTransportContractTests`, `FitMatchClosetSyncCoordinatorTests`, `FitMatchLinkedClosetRoundTripTests`, and the exact-identity `NOT_COMPARABLE` resolver regression. The final bundle is `/tmp/FitMatchSimpleFitMatchRoundTripFinal5.xcresult`. `xcrun swiftc -parse` across all modified production Swift files passed.
- Production was contacted only through the configured public list RPC as an anonymous **read-only** probe. It returned `401 permission denied for function fitmatch_vnext_list_closet_items`; this confirms the endpoint/RPC exists and anonymous access is denied, but it cannot inspect authenticated list DTOs, function bodies, triggers, or schema. No Production data read/write, DDL, migration, RPC replacement, deployment, or account mutation occurred. The candidate remains unapplied pending an authorized authenticated read-only comparison.

## 2026-09-09 Simple FitMatch Phase 1 — authorized Production metadata audit follow-up

- After explicit authorization, Production was inspected through **read-only catalog/function metadata queries only**. No business rows were read; no DDL/DML, migration/RPC replacement, deployment, or account mutation occurred. The live schema includes `closet_items.measurement_mode`, per-row `closet_item_measurements.value_source`, `source_measurement_code`, and `fitmatch_measurement_code`; it also has the exact Product/variant/size hierarchy trigger and the authenticated public upsert/update/list bridges.
- The former “no schema change” conclusion is superseded. Live `closet_item_measurements_one_semantic_chk` requires exactly one of `source_measurement_code` / `fitmatch_measurement_code`, and `validate_closet_measurement_mode()` rejects any source code on a CANONICAL row. That conflicts with the Phase 1 contract: a corrected imported chest must keep both canonical FitMatch meaning and its verified raw source identity. The live list DTO also omits `source_measurement_code`, while the existing live generic update path recreates submitted rows as source-null `USER_MANUAL`; both would lose mixed provenance on later hydration.
- `supabase/sql/127_simplefitmatch_linked_closet_round_trip_candidate.sql` is therefore a **migration candidate only** (not an applied migration). It leaves the live mutually-exclusive check and `validate_closet_measurement_mode()` unchanged, then adds nullable `source_measurement_code_snapshot` with an FK plus a separate additive trigger. SOURCE_NATIVE keeps its existing source-only contract; a CANONICAL linked Closet row stores its verified parser origin in the new snapshot field. The linked helper resolves the selected size with the existing `canonical_measurements_for_size_with_context` function using the user-selected Closet tuple (not a client label heuristic), validates each retailer row against that exact ProductSize, retains source identity for edited imported rows, keeps a newly added user value snapshot-null end to end, rejects a registered wrong-size/wrong-meaning source identity, and returns the existing DTO `source_measurement_code` as `coalesce(snapshot, native_source)`. No taxonomy, parser mapping, ProductSize mutation, or global classification mutation is proposed.
- The disposable PostgreSQL 17 preimage recreates the live source-measurement FK, mutually-exclusive check, and trigger before the candidate. The full public-RPC fixture → existing migrations → preimage → candidate → validation sequence passed after this correction: untouched importer values retain registered identities; chest `55.25 → 56 USER_MANUAL` retains its chest snapshot while its original semantic source column remains NULL; shoulder/back/sleeve remain retailer values; added `hem_width` is `USER_MANUAL` with source JSON `null` and snapshot NULL; category-only/re-edit, exact hierarchy, cross-user denial, invalid unit/NaN/infinity/out-of-range rejection, forged registered source identity, Product/ProductSize immutability, privilege matrix, rollback, and unchanged original canonical semantic exclusivity all pass. Post-validation returns to the fixture baseline (`linked_rows=3`, original fixture chest `50`).
- Final static parse over every modified Swift production/test file and `git diff --check` pass. The previously completed focused simulator bundle remains `57 PASS / 0 FAIL`; a later broad resolver run reproduced its known five unrelated Result→Closet variant/preferred-size failures, and subsequent local Xcode test/build wrappers stalled before creating a new bundle, so those stalled reruns are not counted as test passes.
- Separate unrelated Production metadata finding: `fitmatch_vnext.manual_cross_comparison_rules` and `fitmatch_vnext.classification_axis_value_authority` currently have RLS disabled. This Phase deliberately does not alter comparison/classification assets or Production; track the exposure in a separately authorized security change.
## 2026-09-09 Simple FitMatch Goal 1 — DEV migration checkpoint

- Started clean at `a55542f40b48d63e4dfeeadd6ecb278429d453c1`, matching `origin/simplefitmatch`. This checkpoint is committed/pushed before returning the local checkout to `connectDB`.
- Exact linked runtime no longer waits for classification/recovery. Exact-code drafts preserve unsaved category/detail and same-size edits, reseed only on real size identity change, and preserve owned same-size snapshots after retailer ProductSize drift. Receipt reconciliation checks identity, classification, value, unit, provenance, and source identity.
- DEV `hnkplvyegonlhumlejst` received migration `20260909005903_simplefitmatch_linked_closet_round_trip`, adding detail/source snapshot storage, validation, linked helpers, and compatible public upsert/update/list bridges. Manual/legacy/connectDB requests retain the existing fallback; existing semantic/hierarchy validation was not weakened.
- Disposable PostgreSQL public-RPC validation, DEV metadata/privilege checks, Swift parse, whitespace, and protected UI checks passed. Latest Xcode type build/XCTest and authenticated Swift↔DEV E2E are **NOT RUN**, so Goal 1 remains **PARTIAL** and the next Goal is **NO**. See `Docs/SimpleFitMatchPhase1RoundTripReport-20260909.txt`.
