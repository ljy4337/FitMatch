...
enum MusinsaFallbackTableParser {
    ...
    static func parseGrid(
        ...
        let allowsSingleNamedSize = normalizedContext.contains("실측")
    ...
    private static func makeSizes(
        ...
        if allowsSingleNamedSize,
           let onlySize = parsed.first,
           ["FREE", "ONE"].contains(SizeTokenNormalizer.normalizedKey(for: onlySize.name)) {
            return parsed
        }
        return nil
    }
    ...
    private static func isSizeValue(_ text: String) -> Bool {
        SizeTokenNormalizer.isValid(text)
    }
